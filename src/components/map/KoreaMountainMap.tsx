...
interface KoreaMountainMapProps {
  ...
  focusMountain?: MountainWithProgress;
}

function buildHtml(mountains: MountainWithProgress[], focusMountain?: MountainWithProgress): string {
  const data = JSON.stringify(mountains.map(({ id, nameKo, latitude, longitude, climbed }) => ({ id, nameKo, latitude, longitude, climbed })))
    .replaceAll('<', '\\u003c');
  const initialLatitude = focusMountain?.latitude ?? 36.35;
  const initialLongitude = focusMountain?.longitude ?? 127.8;
  const initialZoom = focusMountain ? 13 : 7;
  const focusId = JSON.stringify(focusMountain?.id ?? '');
  return `<!doctype html>
  ...
  <style>html,body,#map{height:100%;margin:0;background:#E9DFC8}.leaflet-control-attribution{font-size:9px}.mountain-marker{width:32px;height:32px;border-radius:18px;background:#173F32;border:3px solid #fff;color:#fff;display:flex;align-items:center;justify-content:center;box-shadow:0 3px 9px #0004;font-size:16px}.mountain-marker.done{background:#B74D45;border-color:#FFE8D8;font-weight:900}.mountain-marker.focused{outline:5px solid #F4C95D88;transform:scale(1.12)}.leaflet-tooltip{font-family:-apple-system,BlinkMacSystemFont,sans-serif;font-weight:800;border:0;border-radius:8px;box-shadow:0 2px 8px #0002}</style></head>
  <body><div id="map"></div><script src="https://unpkg.com/leaflet@1.9.4/dist/leaflet.js"></script><script>
  const focusId=${focusId};
  const map=L.map('map',{zoomControl:false,minZoom:6,maxZoom:15}).setView([${initialLatitude},${initialLongitude}],${initialZoom});
  ...
  mountains.forEach(m=>{const icon=L.divIcon({className:'',html:'<div class="mountain-marker '+(m.climbed?'done ':'')+(m.id===focusId?'focused':'')+'">'+(m.climbed?'✓':'▲')+'</div>',iconSize:[38,38],iconAnchor:[19,19]});const marker=L.marker([m.latitude,m.longitude],{icon}).addTo(map);marker.bindTooltip(m.nameKo,{direction:'top',offset:[0,-16]});if(m.id===focusId){marker.openTooltip()}marker.on('click',()=>window.ReactNativeWebView.postMessage(JSON.stringify({type:'select',id:m.id})));});
  </script></body></html>`;
}

export function KoreaMountainMap({ mountains, onSelect, focusMountain, children }: PropsWithChildren<KoreaMountainMapProps>) {
  const [loading, setLoading] = useState(true);
  const [failed, setFailed] = useState(false);
  const html = useMemo(() => buildHtml(mountains, focusMountain), [mountains, focusMountain]);
